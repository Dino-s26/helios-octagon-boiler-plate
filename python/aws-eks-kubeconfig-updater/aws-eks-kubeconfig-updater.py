...
def list_eks_clusters(profile, region):

    boto3.setup_default_session(profile_name=profile, region_name=region)

    eks = boto3.client('eks')
    get_eks_clusters = eks.list_clusters()

    json_obj = json.dumps(get_eks_clusters, indent=4, default=str)
    eks_clusters = json_obj[1]

    try:
        print(eks_clusters)
        console.log(f"[green]Done getting data for EKS Clusters !")

    except Exception as e:
        console.log(f"Error while running command : {e}")
        logging.error(f"An error occurred while getting EKS Clusters data: {e}")
...
def main():
    ...
    try:
        ...
        list_eks_clusters(profile, region)
    except Exception as e:
        console.log(f"Error while running command : {e}")
